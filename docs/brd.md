...
- **Demonstrate cross-skill causation:** Show that profile data quality directly drives audience membership — upsert a profile with the right custom property, and the matching segment updates automatically. No explicit add-member call. This makes the connection between the two skills visible and demonstrates a CDP concept that most integrations treat as implicit.
...
- Cross-skill interaction scenario: profile property upsert → automatic segment membership update, demonstrating causation between Manage Profiles and Manage Audiences skills (FSD Section 4)
