...
**Platform:** Klaviyo (free tier). **Scenarios:** Manage Profiles, Manage Audiences. **Format:** Standalone HTML prototypes + Python/FastAPI backend (for MCP generation and live API calls). **Not** a production tool.
...
- **Domain skill layer** (informs stages 2–3): codified priors about how profile and audience management APIs typically behave across platforms
...
- Scenario selector: Manage Profiles, Manage Audiences (both default selected)
...
- Skill warnings surfaced (10 total: 4 Manage Profiles + 6 Manage Audiences — see FSD Sections 2.3 and 3.4)
...
Two markdown files encoding prior knowledge:
...
- Lists vs. Segments distinction; segment membership = computed, not writable
- Gotchas: two-path list add-member consent trap, profile ID vs. email requirement, segment daily cap, empty segment = data quality problem not config problem
...
- Same domain knowledge, restructured for per-tool LLM guidance at call time
- Sections: Auth, Manage Profiles (per operation), Manage Audiences — Lists (per operation), Manage Audiences — Segments (per operation), Cross-skill interaction, Known Live Issues
...
## Open questions (from FSD Section 9)
